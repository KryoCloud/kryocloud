...
public record InstanceSnapshot(UUID requestId, String serviceId, String groupName, String templateName, CloudServiceType serviceType, int port, int maxMemoryMb, boolean staticService, Path workingDirectory, ICloudInstance instance, CloudServiceState state, Instant startedAt) {
    ...
    public InstanceSnapshot withState(CloudServiceState newState) {
        if (newState == null) {
            throw new IllegalArgumentException("newState must not be null");
        }

        return new InstanceSnapshot(requestId, serviceId, groupName, templateName, serviceType, port, maxMemoryMb, staticService, workingDirectory, instance, newState, startedAt);
    }
}
