...
public final class CallerRunsRejectionHandler implements SchedulerRejectionHandler {
    ...
    public <T> void handle(Runnable task, CompletableFuture<T> future, String taskName) {
        if (!allowOnEventLoop && isNettyEventLoop()) {
            TaskRejectedException ex = new TaskRejectedException(
                    taskName,
                    "queue full, policy=CALLER_RUNS blocked on EventLoop, falling back to ABORT"
            );
            future.completeExceptionally(ex);
            LOG.log(Level.WARNING,
                    "CallerRuns rejected on Netty EventLoop for task: " + taskName);
            return;
        }
    ...
    private static boolean isNettyEventLoop() {
        ...
        return threadName.contains("nioevento")
                || threadName.contains("epoll")
                || threadName.contains("kqueue")
                || threadName.contains("event-loop");
    }
}
